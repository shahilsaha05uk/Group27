...
namespace UnrealBuildTool
{
	class ClangTidyToolChain : VCToolChain
	{
		...
		HashSet<string> KnownClangConfigPaths;
...
#endif
		{
#if HAS_UNREAL_ENGINE_5_2_SEMANTICS
            string InstallFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "ClangTidyForUnrealEngine-15");
#else
			...
			KnownClangConfigPaths = new HashSet<string>();

			if (!File.Exists(ClangTidyAnalyzerFile.FullName))
			{
#if HAS_UNREAL_ENGINE_5_3_SEMANTICS
                Logger.LogWarning("Unable to locate clang-tidy. Ensure the plugin has copied across the required files successfully. Expected to find clang-tidy installed at: '{0}'. clang-tidy checks will not run.", ClangTidyAnalyzerFile);
#else
			...
			{
#if HAS_UNREAL_ENGINE_5_3_SEMANTICS
                Logger.LogWarning("Unable to locate clang-cl. Ensure the plugin has copied across the required files successfully. Expected to find clang-tidy installed at: '{0}'. clang-tidy checks will not run.", ClangClCompilerFile);
#else
			...
			{
#if HAS_UNREAL_ENGINE_5_3_SEMANTICS
                Logger.LogWarning("Unable to locate ClangTidySystem.lua. Ensure the plugin has copied across the required files successfully. Expected to find ClangTidySystem.lua installed at: '{0}'. clang-tidy checks will not run.", ClangTidySystemLuaFile);
#else
			...
			{
#if HAS_UNREAL_ENGINE_5_3_SEMANTICS
                Logger.LogInformation("clang-tidy will not run because this is a Live Coding compile operation.");
#else
		...
		private static string[] ReadEventualFileLines(string FilePath)
		{
			for (int i = 0; i < 100; i++)
			{
				try
			...
			throw new InvalidOperationException($"ReadEventualFileLines failed to read file from path: {FilePath}");
		}
		...
		{
			return (Reference.HasExtension(".response") || Reference.HasExtension(".rsp")) && !IsClangVariantFilename(Reference);
		}
...
#endif
		{
			...
			}))
			{
				...
				if (Action.PrerequisiteItems.FirstOrDefault(x => IsResponseFile(x.Location)) == null)
				{
					continue;
				}
...
");
		...
		private FileItem GenerateClangResponseFile(FileItem OriginalResponseFile, IActionGraphBuilder Graph, ref List<FileReference> AdditionalDependencies, bool bSilenceWarnings, out FileItem DependencyListFile)
		{
			...
			ResponseFileLinesList = new List<string>
			{
				...
				"/clang:-fms-compatibility-version=19." + EnvVars.ToolChainVersion.GetComponent(1),
...
#endif
		{
			...
			else
			{
				...
				{
					var Lines = ReadEventualFileLines(Item.AbsolutePath);
					foreach (var Line in Lines)
					{
						if (Line.Contains("#define " + ModuleName + "_ENABLE_CLANG_TIDY 0"))
						{
							bIsClangTidyEnabled = false;
							bDidOverride = true;
							break;
			...
			foreach (var PreprocessAction in PreprocessActions)
			{
				...
				if (ProducesPCHFile(PreprocessAction))
				{
					...
					if (PreprocessAction.PrerequisiteItems.FirstOrDefault(x => IsResponseFile(x.Location)) == null)
					{
						continue;
					}
...
#if HAS_UNREAL_ENGINE_5_2_SEMANTICS
                    foreach (var Item in PreprocessAction.PrerequisiteItems.Where(x => !IsResponseFile(x.Location)))
                    {
                        GeneratePCHAction.PrerequisiteItems.Add(Item);
                    }
                    foreach (var Item in AdditionalDependencies.Select(x => GetSafeFileItem(x)))
                    {
                        GeneratePCHAction.PrerequisiteItems.Add(Item);
                    }
#else
			...
			// Detect .clang-tidy file next to ClangTidy.lua file.
			string ClangTidyConfigPath = null;
			if (System.IO.Path.GetDirectoryName(ClangTidyLuaPath) != null)
			{
				ClangTidyConfigPath = System.IO.Path.Combine(
					System.IO.Path.GetDirectoryName(ClangTidyLuaPath),
					".clang-tidy");
				if (ClangTidyConfigPath != null &&
					!System.IO.File.Exists(ClangTidyConfigPath))
				{
			...
			if (ClangTidyConfigPath != null)
			{
				KnownClangConfigPaths.Add(ClangTidyConfigPath);
			}
			...
			else
			{
				foreach (FileItem Item in CompileEnvironment.ForceIncludeFiles)
				{
					...
					foreach (var Line in Lines)
					{
						if (Line.Contains("#define " + ModuleName + "_CLANG_TIDY_DISPLAY_QUERY_COMMANDS 1"))
						{
			...
			foreach (var PreprocessAction in PreprocessActions)
			{
				...
				if (!DependencyListDictionary.ContainsKey(PreprocessAction))
				{
					continue;
				}
				...
				AnalyzeAction.CommandArguments = string.Format(
					"--lua-script-path=\"{5}\" --lua-script-path=\"{3}\" --checks=-*{6} --touch-path=\"{4}\" --quiet \"--header-filter=.*/{2}/.*\" \"-p={0}\" \"{1}\"",
					CompileCommandsJson.Location,
					SourceFileItem.Location,
					ModuleName,
					ClangTidyLuaPath,
					...
					ClangTidyConfigPath != null
						? string.Format(" --config-file=\"{0}\"", ClangTidyConfigPath)
						: string.Empty);
				...
				if (ClangTidyConfigPath != null)
				{
					AnalyzeAction.PrerequisiteItems.Add(GetSafeFileItem(new FileReference(ClangTidyConfigPath)));
				}
				AnalyzeAction.bCanExecuteRemotely = true;
				// AnalyzeAction.bCanExecuteRemotelyWithSNDBS = true;
#if HAS_UNREAL_ENGINE_5_2_SEMANTICS
                foreach (var Item in PreprocessAction.PrerequisiteItems)
                {
                    if (IsResponseFile(Item.Location) || IsPCHFile(Item.Location))
                    {
                        var SafeItem = GetSafeFileItem(GetClangVariantFilename(Item.Location));
                        AnalyzeAction.PrerequisiteItems.Add(SafeItem);
                    }
                    if (IsSourceFile(Item.Location))
                    {
                        var SafeItem = GetSafeFileItem(Item.Location);
                        AnalyzeAction.PrerequisiteItems.Add(SafeItem);
                    }
                }
#else
		...
		public override void GetExternalDependencies(HashSet<FileItem> ExternalDependencies)
		{
			...
			foreach (var FilePath in this.KnownClangConfigPaths)
			{
				ExternalDependencies.Add(GetSafeFileItem(new FileReference(FilePath)));
			}
		}
...
#if HAS_UNREAL_ENGINE_5_2_SEMANTICS
            foreach (var Item in ClangFiles)
            {
                ((Action)LinkActions[0]).PrerequisiteItems.Add(Item);
            }
#else
