...
$InstalledClangVersion = "13"

# Compute the local build flags and clang-tidy version based on the engine.
$LocalBuildFlags = @()
$RequiresGenericPlatformProcessPatch = $false
if ((Test-Path "$EngineDir\Engine\Source\Programs\UnrealBuildTool\Platform\Windows\VCToolChain.cs") -and
    ((Get-Content -Raw -Path "$EngineDir\Engine\Source\Programs\UnrealBuildTool\Platform\Windows\VCToolChain.cs").Contains("protected override CPPOutput CompileCPPFiles"))) {
    $LocalBuildFlags += "#define HAS_UNREAL_ENGINE_5_2_SEMANTICS"
    $InstalledClangVersion = "15"
    $RequiresGenericPlatformProcessPatch = $true
}
if ((Test-Path "$EngineDir\Engine\Source\Programs\AutomationTool\AutomationUtils\CommandUtils.cs") -and
    ((Get-Content -Raw -Path "$EngineDir\Engine\Source\Programs\AutomationTool\AutomationUtils\CommandUtils.cs").Contains("https://tinyurl.com/bp96bk2r"))) {
    $LocalBuildFlags += "#define HAS_UNREAL_ENGINE_5_3_SEMANTICS"
}

$InstalledFolderName = "ClangTidyForUnrealEngine-$InstalledClangVersion"
...
if ($OutOfDateBinaries) {
    ...
    try {
        $Metadata = (Invoke-WebRequest -UseBasicParsing -Uri "$LicenseServerURL/api/licenses/download/metadata" -Method Post -Body @{
            ...
            productVersion = "redpoint-clang-$InstalledClangVersion";
...
# Compute the local build flags based on the engine.
$LocalBuildFlags = @()
if ((Test-Path "$EngineDir\Engine\Source\Programs\UnrealBuildTool\Platform\Windows\VCToolChain.cs") -and
    ((Get-Content -Raw -Path "$EngineDir\Engine\Source\Programs\UnrealBuildTool\Platform\Windows\VCToolChain.cs").Contains("protected override CPPOutput CompileCPPFiles"))) {
    $LocalBuildFlags += "#define HAS_UNREAL_ENGINE_5_2_SEMANTICS"
...
foreach ($File in $FilesToInstall) {
    ...
    $LocalContent = $LocalContent.Replace("// #replace", $LocalBuildFlags -join "`n")
...
# Check if we need to patch GenericPlatformProcess.h (on Unreal Engine 5.2)
# This file has a bad compiler check that doesn't include the MSVC code path when
# being built by clang-cl (because both _MSC_VER and __clang__ are defined).
if ($RequiresGenericPlatformProcessPatch) {
    $GenericPlatformProcess = (Get-Content -Raw "$EngineDir\Engine\Source\Runtime\Core\Public\GenericPlatform\GenericPlatformProcess.h")
    if ($GenericPlatformProcess.Contains("#if defined(_MSC_VER) && !defined(__clang__)")) {
        $GenericPlatformProcess = $GenericPlatformProcess.Replace("#if defined(_MSC_VER) && !defined(__clang__)", "#if defined(_MSC_VER) // && !defined(__clang__)")
        Set-RetryableContent -Path "$EngineDir\Engine\Source\Runtime\Core\Public\GenericPlatform\GenericPlatformProcess.h" -Value $GenericPlatformProcess
    }
}
...
# Check if we need to patch UEBuildPlatformSDK.cs
if (Test-Path "$EngineDir\Engine\Source\Programs\Shared\EpicGames.Build\UEBuildPlatformSDK.cs") {
    $UEBPSDK = (Get-Content -Raw "$EngineDir\Engine\Source\Programs\Shared\EpicGames.Build\UEBuildPlatformSDK.cs")
    $OriginalUEBPSDK = $UEBPSDK
    if ($UEBPSDK.Contains("throw new Exception(string.Format(`"Re-registering SDK for {0}. All Platforms must have a unique SDK object`", PlatformName));")) {
        $UEBPSDK = $UEBPSDK.Replace("throw new Exception(string.Format(`"Re-registering SDK for {0}. All Platforms must have a unique SDK object`", PlatformName));", "return;")
    }
    if ($UEBPSDK.Trim() -ne $OriginalUEBPSDK.Trim()) {
        if (!(Test-Path "$EngineDir\Engine\Source\Programs\Shared\EpicGames.Build\UEBuildPlatformSDK.cs.backup")) {
            Copy-Item -Force "$EngineDir\Engine\Source\Programs\Shared\EpicGames.Build\UEBuildPlatformSDK.cs" "$EngineDir\Engine\Source\Programs\Shared\EpicGames.Build\UEBuildPlatformSDK.cs.backup"
        }
        Set-RetryableContent -Path "$EngineDir\Engine\Source\Programs\Shared\EpicGames.Build\UEBuildPlatformSDK.cs" -Value $UEBPSDK
        $NeedsRebuild = $true
    }
}
...
if ($NeedsRebuild) {
    ...
    try {
        ...
        foreach ($CopyFile in $CopyFiles) {
            if ($IsUnrealEngine5) {
                if ((Test-Path "$EngineDir\Engine\Binaries\DotNET\UnrealBuildTool\$($CopyFile)") -and $CopyFile.EndsWith("UnrealBuildTool.pdb")) {
                ...
                if ((Test-Path "$EngineDir\Engine\Binaries\DotNET\AutomationTool\$($CopyFile)") -and $CopyFile.EndsWith("UnrealBuildTool.pdb")) {
                ...
                if ((Test-Path "$EngineDir\Engine\Binaries\DotNET\AutomationTool\AutomationUtils\$($CopyFile)") -and $CopyFile.EndsWith("UnrealBuildTool.pdb")) {
